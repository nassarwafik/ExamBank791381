...
describe("student side — preset avatars only, teacher photo shown first", () => {
  ...
  it("the identity card reads the OWN photo once (only when the server reports a version) and shows it over the preset; without a version no request", async () => {
    ...
    const { rerender } = render(<StudentIdentityCard student={{ ...base, profilePhoto: null }} classroom={null} displayName="ليان" stageGroup={null} token="stok" onChangeAvatar={vi.fn()} />);
    expect(calls.length).toBe(0);
    expect(document.querySelector(".eb-profile-avatar.is-avatar")).toBeTruthy();
    rerender(<StudentIdentityCard student={{ ...base, profilePhoto: { version: 3, updatedAt: "" } }} classroom={null} displayName="ليان" stageGroup={null} token="stok" onChangeAvatar={vi.fn()} />);
