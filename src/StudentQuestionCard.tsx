...
export default function StudentQuestionCard({q,index,id,answer,onChoice,onSeq,onTable,onText,onField,disabled}:Props){
 ...
 const noopField=()=>{};
 return <article className={"iex-q "+(answered(a)?"done":"")}><div className="iex-node">{index+1}</div><div className="iex-card"><div className="iex-qhead"><span>{t==="multiplechoice"?"اختيار من متعدد":t==="truefalse"?"صح أو خطأ":t==="multitruefalse"?"صح/خطأ متعدد":t==="clifill"?"أوامر CLI":t==="matching"?"طابق":t==="ordering"?"رتّب العناصر":(table||fieldType)?"أكمل الجدول":seq?"أكمل الناقص":"سؤال"}</span><strong>{q.marks} علامة</strong></div><p className="iex-qtext">{promptText(q.text)}</p>
  {imageList(q).map((im,n)=>im.dataUrl?<img className="iex-image" src={im.dataUrl} alt={"صورة السؤال "+(index+1)} key={n}/>:null)}
  {(t==="multiplechoice"||t==="truefalse")&&<div className="iex-options">{(q.options||[]).map((o,n)=><label className={"iex-option "+(a?.kind==="choice"&&a.index===n?"selected":"")} key={n}><input type="radio" name={id} checked={a?.kind==="choice"&&a.index===n} onChange={()=>onChoice(n)} disabled={disabled}/><span className="iex-pick">{a?.kind==="choice"&&a.index===n&&<IconCheck size={14}/>}</span><b>{o.text||o.label||o.value||""}</b></label>)}</div>}
