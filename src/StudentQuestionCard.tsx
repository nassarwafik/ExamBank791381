...
export default function StudentQuestionCard({q,index,id,answer,onChoice,onSeq,onTable,onText,disabled}:Props){
 ...
 return <article className={"iex-q "+(answered(a)?"done":"")}><div className="iex-node">{index+1}</div><div className="iex-card"><div className="iex-qhead"><span>{t==="multiplechoice"?"اختيار من متعدد":table?"أكمل الجدول":seq?"أكمل الناقص":"سؤال"}</span><strong>{q.marks} علامة</strong></div><p className="iex-qtext">{promptText(q.text)}</p>
