...
type Props={
 ...
 disabled?:boolean;
 /** UX-7b-1: question/part context ("السؤال 3", "السؤال 3 — الجزء أ") prefixed to the accessible names of unlabeled controls. */
 labelPrefix?:string;
};
...
function BooleanSelect({value,onChange,name,disabled,label}:{value:string;onChange:(v:string)=>void;name:string;disabled?:boolean;label?:string}){
 return <select className="iex-cell-select iex-tf-select" name={name} aria-label={label} value={value} onChange={e=>onChange(e.target.value)} disabled={disabled}>
...
export default function QuestionField({q,idBase,values,onField,disabled,labelPrefix}:Props){
 const type=String(q.type||q.presentationType||"").toLowerCase();
 const fields=q.fields||[];
 const get=(fid:string)=>asString(values[fid]);
 const named=(text:string)=>(labelPrefix?labelPrefix+" — ":"")+text;

 // multiTrueFalse: one independently-answered صحيح/غير صحيح per statement row.
 if(type==="multitruefalse"){
  return <div className="iex-mtf">{fields.map((f,i)=>{const fid=fieldId(f,i);const statement=f.statement||f.label||("عبارة "+(i+1));return <div className="iex-mtf-row" key={fid}><span className="iex-mtf-text">{statement}</span><BooleanSelect name={idBase+"-"+fid} label={named(statement)} value={get(fid)} onChange={v=>onField(fid,v)} disabled={disabled}/></div>;})}</div>;
 }
 ...
 if(type==="clifill"&&q.cli){
  ...
  return <pre className="iex-cli">{parts.map((chunk,i)=>{
   ...
   return <input className="iex-cli-input" key={i} aria-label={named("الفراغ "+fid)} value={get(fid)} onChange={e=>onField(fid,e.target.value)} placeholder="…" disabled={disabled} spellCheck={false} autoCapitalize="off" autoCorrect="off"/>;
  })}</pre>;
 }
 ...
 if(type==="tablefill"&&(q.tableHeaders||q.tableRows)){
  ...
  return <div className="iex-table-wrap"><table><thead><tr>{Array.from({length:colCount},(_,c)=><th key={c} scope="col">{headers[c]||""}</th>)}</tr></thead><tbody>{Array.from({length:rowCount},(_,r)=><tr key={r}>{Array.from({length:colCount},(_,c)=>{
   const f=fieldAt.get(r+":"+c);
   if(!f)return <td key={c}>{rows[r]?.[c]??""}</td>;
   const fid=String(f.id);
   const kind=String(f.kind||"text").toLowerCase();
   // Accessible name from the grid context: "<row label> — <column header>" (falls back to positions).
   const rowLabel=rows[r]?.find((cell,ci)=>ci!==c&&String(cell||"").trim())||("الصف "+(r+1));
   const cellName=named(rowLabel+" — "+(headers[c]||("العمود "+(c+1))));
   return <td key={c}>{
    kind==="boolean"?<BooleanSelect name={idBase+"-"+fid} label={cellName} value={get(fid)} onChange={v=>onField(fid,v)} disabled={disabled}/>:
    kind==="select"?<select className="iex-cell-select" aria-label={cellName} value={get(fid)} onChange={e=>onField(fid,e.target.value)} disabled={disabled}><option value="">— اختر —</option>{(f.options||[]).map((o,k)=><option key={k} value={optText(o)}>{optText(o)}</option>)}</select>:
    <input className="iex-cell" aria-label={cellName} value={get(fid)} onChange={e=>onField(fid,e.target.value)} placeholder="اكتب الإجابة" disabled={disabled}/>
   }</td>;
  })}</tr>)}</tbody></table></div>;
 }
