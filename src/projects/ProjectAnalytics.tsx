...
// A small palette cycled by track index so every project's tracks get stable, distinct colors.
const TRACK_COLORS = ["rgba(37,99,235,.82)", "rgba(13,148,136,.82)", "rgba(217,119,6,.82)", "rgba(147,51,234,.82)"];
...
export default function ProjectAnalytics({ token, projectCode, classId, tracks }: Props) {
  ...
  const perStudentData = useMemo(() => data && ({
    ...
    datasets: [{ label: "التقدم العام", data: data.perStudent.map(s => s.overall), backgroundColor: TRACK_COLORS[0], borderRadius: 6 }]
  }), [data]);

  const trackCompareData = useMemo(() => data && ({
    labels: data.perStudent.map(s => s.name),
    datasets: tracks.map((t, i) => ({ label: (t.icon ? t.icon + " " : "") + t.title, data: data.perStudent.map(s => s.trackProgress[t.trackId] || 0), backgroundColor: TRACK_COLORS[i % TRACK_COLORS.length], borderRadius: 6 }))
  }), [data, tracks]);

  const stageData = useMemo(() => {
    if (!data) return null;
    const rows = data.stageCompletion.filter(s => s.track === stageTrack);
    return { labels: rows.map(s => s.stageId), datasets: [{ label: "نسبة الاعتماد", data: rows.map(s => s.approvedPct), backgroundColor: TRACK_COLORS[1], borderRadius: 5 }] };
  }, [data, stageTrack]);

  const bucketData = useMemo(() => data && ({
    labels: ["0–25", "26–50", "51–75", "76–99", "100"],
    datasets: [{ label: "عدد الطلاب", data: [data.buckets["0-25"], data.buckets["26-50"], data.buckets["51-75"], data.buckets["76-99"], data.buckets["100"]], backgroundColor: TRACK_COLORS[0], borderRadius: 6 }]
  }), [data]);
  ...
  return (
    <>
      <div className="p794-analytics">
        ...
        <section className="platform-card"><h3>مقارنة المسارات</h3><div className="p794-chart">{trackCompareData && <Bar data={trackCompareData} options={barOptions} />}</div></section>
        <section className="platform-card">
          <div className="platform-card-heading"><h3>نسبة إنجاز كل مرحلة</h3>
            <div className="analytics-view-tabs">
              {tracks.map(t => <button key={t.trackId} type="button" className={"analytics-view-tab " + (stageTrack === t.trackId ? "active" : "")} onClick={() => setStageTrack(t.trackId)}>{trackIcon(t.icon)}</button>)}
            </div>
          </div>
      ...
      <section className="platform-card p794-heatmap-card"><h3>الخريطة الحرارية للمراحل</h3><ProjectHeatmap heatmap={data.heatmap} tracks={tracks} groups={groups} /></section>
    </>
  );
}
