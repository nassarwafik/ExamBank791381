...
type Attempt={attemptNumber:number;score:number;totalMarks:number;percentage:number;submittedAt:string;finalized:boolean;manualReviewMarks:number;startedAt?:string;endedAt?:string;endReason?:string;timedOut?:boolean};
...
type StudentResult={studentId:string;studentName:string;studentCode:string;attemptsUsed:number;allowedAttempts:number;dueAtOverride:string|null;attemptStatus?:string;activeAttempt?:ActiveAttempt|null;effectiveAttemptEndsAt?:string;attemptDurationEndsAt?:string;attemptExpired?:boolean;canStartAttempt?:boolean;canWrite?:boolean;timed?:boolean;durationMinutes?:number;attempts:Attempt[];latestResult:Attempt|null};
...
export default function AssignmentsPanel({token,classes,currentExam,onCopyLibraryExamToBuilder}:Props){
 ...
 async function archiveItem(item:Item){
  ...
  try{
   ...
   setItems(x=>x.map(y=>y.assignmentId===item.assignmentId?r.assignment:y));setNotice("✓ تم أرشفة الواجب «"+item.title+"».");
  }catch(e){setError(e instanceof Error?e.message:"تعذر أرشفة الواجب.")}finally{setBusy(false)}
 }
 async function restoreItem(item:Item){setBusy(true);setError("");setNotice("");try{const r=await api<{assignment:Item}>("/api/assignments",{method:"POST",body:JSON.stringify({action:"restore",assignmentId:item.assignmentId})});setItems(x=>x.map(y=>y.assignmentId===item.assignmentId?r.assignment:y));setNotice("✓ تم استعادة الواجب «"+item.title+"».")}catch(e){setError(e instanceof Error?e.message:"تعذر استعادة الواجب.")}finally{setBusy(false)}}
