...
export default function AssignmentsPanel({token,classes,currentExam}:Props){
 ...
 async function loadResults(item=resultsFor){if(!item)return;setBusy(true);setDeadlineFor(null);try{const r=await api<{students:StudentResult[];stats:Stats}>("/api/assignment-results?assignmentId="+encodeURIComponent(item.assignmentId));setResultsFor(item);setResults(r.students||[]);setStats(r.stats||null)}catch(e){setError(e instanceof Error?e.message:"تعذر تحميل النتائج.")}finally{setBusy(false)}}
