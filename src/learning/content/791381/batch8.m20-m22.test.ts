...
describe("Batch 8 — validation, identities, mapping PDF 159–179, cover PDF 158, HARD STOP before PDF 180", () => {
  ...
  it("HARD STOP: every Batch-8 page < 180; the maximum pdfPageStart among ALL converted real bodies is 179; the earlier bodies still stop at 157; manifest ranges 159–165 / 166–168 / 169–179", () => {
    ...
    expect(byId["791381-m05"].lessons[0].pages.map(p => p.source!.pdfPageStart)).toEqual([192, 193, 194]);   // m05 completed in place by Batch 9
...
describe("Batch 8 — pedagogy, provenance, direction", () => {
  it("every page ends with an interactive exercise (practice or practice-table); exactly ONE clarification per page; every practice carries 2 hints + «افحص» incorrect feedback + explanation; review heading + r1–r3 on each module's last page", () => {
    for (const p of pages) {
  ...
  it("counts: 7 / 3 / 11 pages, 13 / 8 / 20 practices, 4 / 1 / 3 worksheets, 7 / 3 / 11 clarifications, 0 / 0 / 2 code blocks, 0 / 1 / 3 simulations; worksheet keys are always one of their options", () => {
    ...
    // m20-l01-p02-q2 and m22-l03-p01-q2 converted from a static fillBlank practice to a dropdown practice-table
    expect(BATCH.map(m => count(m, b => b.type === "practice"))).toEqual([13, 8, 20]);
    expect(BATCH.map(m => count(m, b => b.type === "practice-table"))).toEqual([4, 1, 3]);
...
describe("Batch 8 — loaders, navigation, server registry agreement, publication invariants, m05–m06 untouched", () => {
  ...
  it("navigation: m04 → m20 → m21 → m22 → m05; PDF 157 leads to PDF 159; PDF 165 to 166; PDF 168 to 169; after PDF 179 comes the m05 skeleton (PDF 193)", () => {
    expect(orderedModules(manifest).map(m => m.id).slice(16, 27)).toEqual(["791381-m04", M20, M21, M22, "791381-m23", "791381-m24", "791381-m05", "791381-m25", "791381-m26", "791381-m27", "791381-m06"]);
    ...
    expect([cur?.id, cur?.source!.pdfPageStart]).toEqual(["791381-m23-l01-p01", 180]);   // since Batch 9 the Port Security section follows PDF 179
  });
  it("FRONTEND ↔ SERVER agreement: every module with a body is in the server publication registry with the same title and order (and only those); m20 = 18, m21 = 19, m22 = 20 at the end; skeletons m05–m06 absent", () => {
    ...
    expect(s.slice(17, 20)).toEqual([{ moduleId: M20, title: "Wi-Fi والشبكات اللاسلكية", order: 18 }, { moduleId: M21, title: "IPv6 والمنافذ", order: 19 }, { moduleId: M22, title: "بروتوكول DHCP", order: 20 }]);
    expect(server().findLearningModule("791381", "791381-m06")).not.toBeNull();   // m05 became publishable in Batch 9, m06 in Batch 10
  });
