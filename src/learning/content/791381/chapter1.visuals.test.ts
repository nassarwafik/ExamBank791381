...
describe("Chapter 1 pilot — scope of visual enrichment", () => {
  ...
  it("only the pilot + Batch 2 + Batch 3 + Batch 4 + Batch 5 + Batch 6 + Batch 9 modules contain visual blocks (no other module touched)", () => {
    const withVisual = readdirSync(modulesDir)
      .filter(f => /^m\d+\.ts$/.test(f))
      .filter(f => /type:\s*"visual"/.test(readFileSync(modulesDir + f, "utf8")))
      .sort();
    expect(withVisual).toEqual(["m01.ts", "m02.ts", "m03.ts", "m04.ts", "m06.ts", "m07.ts", "m08.ts", "m09.ts", "m10.ts", "m11.ts", "m12.ts", "m13.ts", "m14.ts", "m15.ts", "m16.ts", "m17.ts", "m18.ts", "m19.ts", "m21.ts", "m22.ts", "m23.ts", "m24.ts", "m26.ts", "m27.ts", "m28.ts"]);
  });
});
