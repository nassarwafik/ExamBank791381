...
describe("Batch 4 — provenance, RTL/LTR, safety, skeletons and registry consistency", () => {
  ...
  it("historical skeletons m03–m06 are untouched (ids, titles, page ids, PDF mappings) and merely shift to orders 13–16 after the batch", () => {
    const byId = Object.fromEntries(manifest.modules.map(m => [m.id, m]));
    expect(["791381-m03", "791381-m04", "791381-m05", "791381-m06"].map(id => byId[id].order)).toEqual([13, 14, 15, 16]);
    expect(byId["791381-m03"].lessons[0].pages.map(p => [p.id, p.title, p.source!.pdfPageStart, p.source!.printedPage])).toEqual([["791381-m03-l01-p01", "منافذ السويتش", 123, 121], ["791381-m03-l01-p02", "برمجة المنافذ من CLI", 124, 122]]);
    ...
    expect(["791381-m03", "791381-m04", "791381-m05", "791381-m06"].map(id => hasModuleContent("791381", id))).toEqual([false, false, false, false]);
    expect(manifest.batches!.find(b => b.id === "b3")!.moduleIds).toEqual(["791381-m13", "791381-m14", "791381-m15", "791381-m16"]);
    expect(manifest.batches!.find(b => b.id === "b4")!.moduleIds).toEqual(["791381-m03", "791381-m04", "791381-m05"]);
  });
  it("FRONTEND ↔ SERVER registry consistency: every manifest module WITH a body is in the server publication registry with the same title and order (and only those); no skeleton is publishable", () => {
    ...
    for (const s of ["791381-m03", "791381-m04", "791381-m05", "791381-m06"]) expect(server.map(m => m.moduleId)).not.toContain(s);
