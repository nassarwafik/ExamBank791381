...
describe("Batch 4 — provenance, RTL/LTR, safety, skeletons and registry consistency", () => {
  ...
  it("historical skeletons m03–m06 are untouched (ids, titles, page ids, PDF mappings) and merely shift after every real module (orders 15–18 since Batch 5)", () => {
    const byId = Object.fromEntries(manifest.modules.map(m => [m.id, m]));
    expect(["791381-m03", "791381-m04", "791381-m05", "791381-m06"].map(id => byId[id].order)).toEqual([15, 16, 17, 18]);
