...
describe("Batch 4 — provenance, RTL/LTR, safety, skeletons and registry consistency", () => {
  ...
  it("historical m03 (completed in place by Batch 6) keeps its two historical pages' ids, titles and PDF mappings; skeletons m04–m06 are untouched and merely shift after every real module (orders 15–18 since Batch 5)", () => {
    ...
    expect(["791381-m03", "791381-m04", "791381-m05", "791381-m06"].map(id => hasModuleContent("791381", id))).toEqual([true, false, false, false]);
  ...
  it("FRONTEND ↔ SERVER registry consistency: every manifest module WITH a body is in the server publication registry with the same title and order (and only those); no skeleton (m04–m06) is publishable", () => {
    ...
    for (const s of ["791381-m04", "791381-m05", "791381-m06"]) expect(server.map(m => m.moduleId)).not.toContain(s);
