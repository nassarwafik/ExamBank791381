...
describe("grammar / parser", () => {
  ...
  it("distinguishes incomplete, invalid and unknown input", () => {
    ...
    for (const u of ["router rip", "show mac-address-table", "ping 8.8.8.8", "line aux 0", "ls -la", "hello"]) expect(parseCommand(u), u).toEqual({ kind: "unknown" });   // `router ospf` became known in Batch 10
...
describe("learner-facing wording is pinned literally (the specification's feedback strings)", () => {
  it("unknown / wrong mode / interface first / not required / retry / correct / done are the exact Arabic strings", () => {
    ...
    expect(CLI_MODE_LABEL).toEqual({ user: "وضع المستخدم", privileged: "وضع الأوامر المتقدّم", global: "وضع الإعداد العام", interface: "وضع إعداد الواجهة", subinterface: "وضع إعداد الواجهة الفرعية", vlan: "وضع إعداد VLAN", dhcp: "وضع إعداد مجموعة DHCP", line: "وضع إعداد خط الدخول", router: "وضع إعداد التوجيه" });
