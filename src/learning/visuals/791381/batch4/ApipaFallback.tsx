...
export default function ApipaFallback({ ariaLabel, reducedMotion, className }: LearningVisualProps) {
  return (
    <svg className={"eb-visual" + (className ? " " + className : "")} viewBox="0 0 380 180"
      ...
      <g className={reducedMotion ? "" : "eb-visual-glow-anim"}>
      ...
      {/* the request goes out; nothing returns */}
      {!reducedMotion ? (
        <g className="eb-visual-packet"><rect x="-6" y="-5" width="12" height="10" rx="2" /><animateMotion dur="1.6s" repeatCount="indefinite" path={`M ${CLIENT.x + 28} ${CLIENT.y} L ${SERVER.x - 28} ${SERVER.y}`} /></g>
      ) : (
