    ...
    <svg className={"eb-visual" + (className ? " " + className : "")} viewBox="0 0 380 200"
      ...
      {TARGETS.map((t, i) => (
        <g key={i} data-target="1">
          <rect className={"eb-visual-node is-target" + (reducedMotion ? "" : " eb-visual-glow-anim")} x={t.x - 50} y={t.y - 17} width="100" height="34" rx="8"
            style={reducedMotion ? undefined : { animationDelay: `${i * 0.9}s` }} />
          <text className="eb-visual-node-label" x={t.x} y={t.y} textAnchor="middle" dominantBaseline="central">{t.label}</text>
        </g>
      ))}
      {/* a marker reaches each target in turn */}
      {!reducedMotion && TARGETS.map((t, i) => (
        <g key={"p" + i} className="eb-visual-packet"><rect x="-5" y="-5" width="10" height="10" rx="2" />
          <animateMotion begin={`${i * 0.9}s`} dur="0.9s" repeatCount="indefinite" path={`M ${THREAT.x} ${THREAT.y} L ${t.x} ${t.y}`} />
        </g>
      ))}
