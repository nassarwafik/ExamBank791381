 ...
 *
 * Motion: the SHARED PREFIX `192.168.1.` is highlighted in turn across all four addresses (a staggered pulse), so the
 * "same prefix = same network" idea is what moves — not merely the hub. The differing host part stays visually
 * distinct (a different colour) throughout. Reduced motion ⇒ NO pulse, but the shared prefix is still colour-
 * distinguished from the host part in the still frame.
 */
const HOSTS = [
...
export default function SameNetwork({ ariaLabel, reducedMotion, className }: LearningVisualProps) {
  return (
    <svg className={"eb-visual" + (className ? " " + className : "")} viewBox="0 0 380 210"
      ...
      <circle className="eb-visual-hub" cx={HUB.x} cy={HUB.y} r="30" />
      ...
      {HOSTS.map((h, i) => (
        <g key={"h" + i}>
          ...
          <text className="eb-visual-token" x={h.x} y={h.y} textAnchor="middle" dominantBaseline="central" direction="ltr">
            {/* the SHARED network prefix — the element that carries the meaningful motion (staggered across devices) */}
            <tspan className={"eb-visual-prefix" + (reducedMotion ? "" : " eb-visual-prefix-anim")}
              style={reducedMotion ? undefined : { animationDelay: `${i * 0.4}s` }}>192.168.1.</tspan>
            <tspan className="eb-visual-hostpart">{h.host}</tspan>
          </text>
        </g>
      ))}
