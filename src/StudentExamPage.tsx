...
export default function StudentExamPage({token,assignment,studentName,className,onBack,onLogout}:Props){
 ...
 return <main className="interactive-exam-page" dir="rtl"><div className="iex-wrap">
  ...
  <section className="iex-flow">{qs.map((q,i)=>{const id=qid(q,i),t=typeOf(q),a=answers[id],table=parseTable(q.text),seq=t==="wordbank"||t==="fillblank"||((q.fields?.length||0)>0&&t!=="open");return <article className={"iex-q "+(answered(a)?"done":"")} key={id}><div className="iex-node">{i+1}</div><div className="iex-card"><div className="iex-qhead"><span>{t==="multiplechoice"?"اختيار من متعدد":table?"أكمل الجدول":seq?"أكمل الناقص":"سؤال"}</span><strong>{q.marks} علامة</strong></div><p className="iex-qtext">{promptText(q.text)}</p>
   ...
   {table&&<div className="iex-table-wrap"><table><thead><tr>{table.headers.map((h,n)=><th key={n}>{h}</th>)}</tr></thead><tbody>{table.rows.map((r,n)=>{const rowOptions=resolveTableRowOptions(q,n);return <tr key={n}><td>{r[0]}</td><td>{rowOptions?<select className="iex-cell-select" value={a?.kind==="table"?String(a.values[n]??""):""} onChange={e=>setTable(id,n,e.target.value)} disabled={submitBusy}><option value="">— اختر —</option>{rowOptions.isBoolean?<><option value="true">صحيح</option><option value="false">غير صحيح</option></>:rowOptions.values.map((v,k)=><option key={k} value={v}>{v}</option>)}</select>:tableCheckbox(q)?<input className="iex-check" type="checkbox" checked={a?.kind==="table"&&Boolean(a.values[n])} onChange={e=>setTable(id,n,e.target.checked)} disabled={submitBusy}/>:<input className="iex-cell" value={a?.kind==="table"?String(a.values[n]??""):""} onChange={e=>setTable(id,n,e.target.value)} placeholder="اكتب الإجابة" disabled={submitBusy}/>}</td></tr>})}</tbody></table></div>}
   {!table&&seq&&(()=>{const bank=getWordBank(q);return <><div className="iex-bank">{bank.map((w,n)=><span key={n}>{w}</span>)}</div><div className="iex-seq">{(q.fields||[]).map((f,n)=><label key={f.id||f.number||n}><span>{f.label||"الحقل "+(n+1)}</span><select value={a?.kind==="sequence"?a.values[n]||"":""} onChange={e=>setSeq(id,n,e.target.value)} disabled={submitBusy}><option value="">— اختر —</option>{bank.map((w,k)=><option key={k} value={w}>{w}</option>)}</select></label>)}</div></>})()}
   {!table&&!seq&&t!=="multiplechoice"&&<textarea className="iex-open" value={a?.kind==="text"?a.value:""} onChange={e=>setAnswers(x=>({...x,[id]:{kind:"text",value:e.target.value}}))} placeholder="اكتب إجابتك هنا..." disabled={submitBusy}/>}
  </div></article>})}</section>
