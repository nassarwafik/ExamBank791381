...
import { normalizeGameDoc, publicActive, startAttempt, applyAnswer, computeResult, mergeBest, streaksOf } from "../src/lib/number-conversion-store.js";
...
const correctBitsFor = (doc, index) => bitsFromValue(doc.active.tasks[index].value);
const wrongBitsFor = (doc, index) => bitsFromValue((doc.active.tasks[index].value + 1) % 256);
...
describe("applyAnswer — grading, hint, reveal, advance", () => {
  it("a correct answer advances with an explanation and increments the streak", () => {
    const doc = fresh(3);
    const r = applyAnswer(doc, { taskId: "t1", bits: correctBitsFor(doc, 0), now: T0 });
  ...
  it("first wrong → a hint and stays on the task; second wrong → reveal + advance, counted incorrect", () => {
    const doc = fresh(3);
    const r1 = applyAnswer(doc, { taskId: "t1", bits: wrongBitsFor(doc, 0), now: T0 });
    ...
    const r2 = applyAnswer(r1.doc, { taskId: "t1", bits: wrongBitsFor(r1.doc, 0), now: T0 });
  ...
  it("a completed / mismatched task cannot be re-answered (anti-farming, refresh-safe)", () => {
    const doc = fresh(3);
    const r = applyAnswer(doc, { taskId: "t1", bits: correctBitsFor(doc, 0), now: T0 });   // now on t2
    const stale = applyAnswer(r.doc, { taskId: "t1", bits: correctBitsFor(doc, 0), now: T0 });
...
describe("completion → server result + best record", () => {
  it("finishing computes correct/total/percentage/bestStreak/elapsed, merges best, clears active", () => {
    ...
    let r = applyAnswer(doc, { taskId: "t1", bits: correctBitsFor(doc, 0), now: T0 }); doc = r.doc;
    r = applyAnswer(doc, { taskId: "t2", bits: wrongBitsFor(doc, 1), now: later(1000) }); doc = r.doc;
    r = applyAnswer(doc, { taskId: "t2", bits: wrongBitsFor(doc, 1), now: later(2000) }); doc = r.doc;
    r = applyAnswer(doc, { taskId: "t3", bits: correctBitsFor(doc, 2), now: later(5000) }); doc = r.doc;
