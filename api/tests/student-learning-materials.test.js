...
describe("student-learning-materials — safe catalog from the class's publication", () => {
  ...
  it("Case 4: storage contains m999 (+ skeleton m06): omitted, default-denied; nothing echoed", async () => {
    const ctx = createMemoryContainer({ [USR("u1")]: student("u1", "cA"), [CLS("cA")]: room("cA", [{ courseId: "791381", visibleModuleIds: [M01, "791381-m999", "791381-m06"] }, { courseId: "999999", visibleModuleIds: ["999999-m01"] }]) });
    const { r } = await call(ctx);
    ...
    expect(JSON.stringify(r.jsonBody)).not.toMatch(/m999|m06|m05|m04|m19|m03|999999/);
  });
...
describe("learning-materials-catalog — teacher catalog endpoint", () => {
  ...
  it("builder-authenticated → the exact publishable catalog (metadata only)", async () => {
    ...
    expect(JSON.stringify(r.jsonBody)).not.toMatch(/m06|pages|lessons|blocks|answer|pdf/i);
  });
