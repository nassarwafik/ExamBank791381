...
const { getClassProjectCodes, getSupportedClassProjectCodes, classHasProject } = require("../lib/project-tracker/class-projects");
...
function classMeta(c) {
  return { classId: c.classId, name: c.name, grade: c.grade || "", schoolYear: c.schoolYear || "", status: normalizeClassStatus(c), projectCodes: getClassProjectCodes(c), studentCount: Array.isArray(c.studentIds) ? c.studentIds.length : 0 };
}
  ...
  handler: async request => {
    try {
      ...
      if (type === "class") {
        ...
        const projects = [];
        for (const pCode of getSupportedClassProjectCodes(c)) {
      ...
      if (type === "student") {
        ...
        const projects = [];
        if (c) for (const pCode of getSupportedClassProjectCodes(c)) {
