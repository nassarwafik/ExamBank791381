    ...
    handler:
      async request => {
        try {
          ...
          // programCodes[] canonical field and drop the legacy programCode so no stale logic depends on it.
          if (action === "setprograms" || action === "setprogram") {
            ...
            try {
              ...
              return {
                status: 200,
                jsonBody: { ok: true, programCodes: getClassProgramCodes(updated) }
              };
            }
            catch (mutateError) {
